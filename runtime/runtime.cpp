...
bool global_runtime_init(env_c &env, source_manager_c& source_manager, cell_memory_manager_t& instruction_memory) {
  if (global_runtime) {
    return true;
  }
...
runtime_c::runtime_c(env_c &env, source_manager_c& source_manager, cell_memory_manager_t& instruction_memory)
  : global_env_(env),
...
cell_c* runtime_c::execute_cell(cell_c* cell, env_c& env) {
  ...
  switch (cell->type) {
    ...
    case cell_type_e::SYMBOL: {
      ...
      if (!loaded_cell) {
        throw exception_c(
          "Symbol not found in environment: " + cell->as_string(),
          cell->locator);
        return nullptr;
      }
