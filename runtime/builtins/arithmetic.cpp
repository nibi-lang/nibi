...
#define PERFORM_OPERATION(___op_fn)                                            \
  ...
  case cell_type_e::INTEGER: {                                                 \
    return global_runtime->get_runtime_memory().allocate(___op_fn<int64_t>(    \
        first_arg->to_integer(),                                               \
        [](cell_c *arg) -> int64_t { return arg->to_integer(); }, list, env)); \
  }                                                                            \
  case cell_type_e::DOUBLE: {                                                  \
    return global_runtime->get_runtime_memory().allocate(___op_fn<double>(     \
        first_arg->to_double(),                                                \
        [](cell_c *arg) -> double { return arg->to_double(); }, list, env));   \
  }                                                                            \
  default: {                                                                   \
