  ...
  LIST_ITER_SKIP_N(1, {
    if (!env.drop((*it)->as_symbol())) {
      throw runtime_c::exception_c(
        "Could not find symbol with name :" + (*it)->as_symbol(), (*it)->locator);
    }
  })
...
namespace {
  cell_c* handle_thrown_error_in_try(std::string message, cell_c& recover_cell, env_c& env) {
...
/*
  ...
  The first param can be a process list, or a data list that will be walked and itered

  The second param is a process list of a data list used to catch the error and deal with it

  Temporarily, we will inject an environment with `$e` set to the error message
