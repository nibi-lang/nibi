...
/*
    ...
    module system. This is to ensure a user can load and drop modules as they see fit.
...
inline void modules_c::load_dylib(
  std::string &name, 
  env_c &module_env,
  ...
  auto rll_cell = allocate_cell(
    ...
    [this, name]() {
      this->loaded_modules_.erase(name);
    },
    target_lib
  )));
...
inline void modules_c::load_source_list(
  std::string &name,
  env_c &module_env,
  std::filesystem::path &module_path,
