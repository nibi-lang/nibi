...
#define NIBI_CONVERSION_TO_TYPE(type, conversion_method) \
  ...
  try { \
    ...
    throw interpreter_c::exception_c( \
        std::string("Invalid argument to integer conversion: ") + value->to_string(), \
        value->locator); \
  } catch (std::out_of_range &e) { \
    throw interpreter_c::exception_c( \
        std::string("Out of range argument to integer conversion: ") + value->to_string(), \
        value->locator); \
  } catch (...) { \
    throw interpreter_c::exception_c( \
        std::string("Unknown error in integer conversion: ") + value->to_string(), \
        value->locator); \
  } \
...
cell_ptr builtin_fn_cvt_to_float(interpreter_c &ci, cell_list_t &list,
                                env_c &env) {
  ...
  if (value->type == cell_type_e::LIST) {
    ...
    cell_list_t data_list;
      ...
      for (auto &cell : as_list) {
        if (count == target) {
          data_list.push_back(allocate_cell(list_info_s{list_types_e::DATA, inner_list}));
          inner_list.clear();
          count = 0;
        }
