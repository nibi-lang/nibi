#include "libnibi/cell.hpp"
#include "libnibi/environment.hpp"
#include "libnibi/keywords.hpp"
#include "macros.hpp"

#include <iostream>

#include <unordered_map>


#include <dlfcn.h>
#include <ffi.h>

namespace {
struct c_types_s {
  const char* type_name;
  nibi::cell_type_e cell_type;
  ffi_type* actual_ffi_type;
};

struct c_data {
  union {
    int64_t i;
    double d;
    float f;
    char* s;
  } data;
  nibi::cell_ptr original_data;
};

std::unordered_map<std::string, c_types_s> command_tag_to_type = {
  {":int", {"int", nibi::cell_type_e::INTEGER, &ffi_type_sint64}},
  {":double", {"double", nibi::cell_type_e::DOUBLE, &ffi_type_double}},
  {":float", {"float", nibi::cell_type_e::DOUBLE, &ffi_type_float}},
  {":str", {"char*", nibi::cell_type_e::STRING, &ffi_type_pointer}}
};
}

namespace nibi {
namespace builtins {

cell_ptr builtin_fn_extern_call(cell_processor_if &ci,
                                cell_list_t &list, env_c &env) {

  /*


      Consider building a dictionary of FAUX functions 
      that encapsulate the management of c types.

      If a library allocates memory, we will want to be able to
      keeo it around, and have that memory managed. 

      Perhaps we could return a FAUX function that has its own env
      containing aberrant data embedded with a struct that details
      the data type, and contains a std::any that holds the data/pointer

  */



  NIBI_LIST_ENFORCE_SIZE(nibi::kw::EXTERN_CALL, >=, 5);
  std::cout << "extern-call: >>> " << list[0]->to_string() << std::endl;


  std::optional<std::string> lib_name = std::nullopt;
  if (list[1]->type != cell_type_e::NIL) {
    lib_name = list[1]->as_string();
  }

  const std::string fn_name = list[2]->as_string();
  auto &arg_types = list[3]->as_list();
  const std::string return_type = list[4]->as_symbol();


  if (list.size() != 5 + arg_types.size()) {
    std::string err = "extern-call: " + 
      std::to_string(arg_types.size()) + " arguments declared, but supplied" + std::to_string(list.size() - 5);
    throw interpreter_c::exception_c(err, list[0]->locator);
  }

  std::vector<c_types_s> arg_info;
  for (auto &arg_type : arg_types) {
    arg_info.push_back(command_tag_to_type[arg_type->as_symbol()]);
  }

  std::vector<c_data> args_supplied;
  for (int i = 5; i < list.size(); i++) {

    c_data cd;
    cd.original_data = ci.process_cell(list[i], env);
    switch(cd.original_data->type) {
      case cell_type_e::INTEGER:
        cd.data.i = cd.original_data->as_integer();
        break;
      case cell_type_e::DOUBLE:
        cd.data.d = cd.original_data->as_double();
        break;
      case cell_type_e::STRING:
        cd.data.s = cd.original_data->as_string().data();
        break;
      default:
        throw interpreter_c::exception_c("extern-call: unsupported type", list[i]->locator);
    }

    args_supplied.push_back(cd);
  }

  std::cout << "extern-call: " << (lib_name.has_value() ? *lib_name : "NULL") << " " << fn_name << " " << return_type << std::endl;
  for (auto &arg_type : arg_info) {
    std::cout << " " << arg_type.type_name;
  }

  std::cout << std::endl;

  for (auto &arg : args_supplied) {
    std::cout << " " << arg.original_data->to_string();
  }

  for(std::size_t i = 0; i < arg_info.size(); i++) {
    if (arg_info[i].cell_type != args_supplied[i].original_data->type) {
      std::string err = "extern-call: argument " + std::to_string(i) + " is of type " + cell_type_to_string(args_supplied[i].original_data->type) + " but should be type " + cell_type_to_string(arg_info[i].cell_type);
      throw interpreter_c::exception_c(err, args_supplied[i].original_data->locator);
    }
  }

  std::cout << std::endl;


  void* lib_handle{nullptr};
  if (lib_name.has_value()) {
    lib_handle = dlopen((*lib_name).c_str(), RTLD_LAZY);
  } else {
    lib_handle = dlopen(nullptr, RTLD_LAZY);
  }

  if (!lib_handle) {
    std::string err = "extern-call: could not open library";
    throw interpreter_c::exception_c(err, list[0]->locator);
  }


  std::cout << "Library loaded!" << std::endl;

  void* fn_ptr = dlsym(lib_handle, fn_name.c_str());

  
  char* error;
  if ((error = dlerror()) != nullptr) {
    std::string err = "extern-call: could not get handle to function: " + fn_name + " " + error;
    throw interpreter_c::exception_c(err, list[0]->locator);
  }

  std::cout << "Function bound!" << std::endl;

  ffi_cif cif;
  ffi_type* ffi_ret_type = command_tag_to_type[return_type].actual_ffi_type;
  ffi_type* ffi_arg_types[20];
  void* ffi_arg_vals[20];
  ffi_status status;

  for(std::size_t i = 0; i < arg_info.size(); i++) {
    ffi_arg_types[i] = arg_info[i].actual_ffi_type;
    ffi_arg_vals[i] = &args_supplied[i].data;
  }

  // Make the call

  status = ffi_prep_cif(&cif, FFI_DEFAULT_ABI, arg_info.size(), ffi_ret_type, ffi_arg_types);

  if (status != FFI_OK) {
    std::string err = "'ffi' call to ffi_prep_cif failed with code: " + std::to_string(status);
    throw interpreter_c::exception_c(err, list[0]->locator);
  }

  c_data return_data;

  ffi_call(&cif, FFI_FN(fn_ptr), &return_data.data, ffi_arg_vals);

  /* close the library */
  dlclose(lib_handle);


  std::cout << return_data.data.i << std::endl;



  /*


Idea:


(io::println ({cffi load_lib}))


  
  (extern-call "lib.so" "add" [:int :int] :int x y)
  
  
  
  (fn printf [s] 
    (extern-call nil "printf" [:str] :int s))



  the `:` symbols are explicit c-type parameters
  that way the user can't just stuff a random
  ass string in and hope it works. 


  */

  return allocate_cell((int64_t)0);
}

} // namespace builtins
} // namespace nibi

  
