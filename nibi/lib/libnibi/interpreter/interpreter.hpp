...
public:
  //! \brief The maximum depth of the context stack
  //! \note This is a safety precaution to prevent infinite recursion
  static constexpr std::size_t MAX_CTX_DEPTH = 4096;
  ...
  void push_ctx() {
    if (ctxs_.size() >= MAX_CTX_DEPTH) {
      throw exception_c("Maximum context depth reached");
    }
    ctxs_.push(ctx_s{});
  }
