...
public:
  static constexpr bool kValue =
      (std::is_trivially_copyable<ExtentsRemoved>::value ||
       !kIsCopyOrMoveConstructible) &&
      (std::is_trivially_copy_assignable<ExtentsRemoved>::value ||
...
template <typename NullPtrT = std::nullptr_t> struct OnlyLiteralZero {
...
public:
  ...
  // Comparisons
  friend constexpr bool
  operator==(strong_equality v, compare_internal::OnlyLiteralZero<>) noexcept {
    return v.value_ == 0;
  }
  friend constexpr bool
  operator!=(strong_equality v, compare_internal::OnlyLiteralZero<>) noexcept {
...
public:
  ...
  // Comparisons
  friend constexpr bool
  operator==(partial_ordering v, compare_internal::OnlyLiteralZero<>) noexcept {
...
// default.
template <typename Compare> struct key_compare_to_adapter {
  using type = Compare;
};

template <> struct key_compare_to_adapter<std::less<std::string>> {
  using type = StringBtreeDefaultLess;
};

template <> struct key_compare_to_adapter<phmap::Less<std::string>> {
  using type = StringBtreeDefaultLess;
};

template <> struct key_compare_to_adapter<std::greater<std::string>> {
  using type = StringBtreeDefaultGreater;
};

#if PHMAP_HAVE_STD_STRING_VIEW
template <> struct key_compare_to_adapter<std::less<std::string_view>> {
  using type = StringBtreeDefaultLess;
};

template <> struct key_compare_to_adapter<phmap::Less<std::string_view>> {
  using type = StringBtreeDefaultLess;
};

template <> struct key_compare_to_adapter<std::greater<std::string_view>> {
  using type = StringBtreeDefaultGreater;
};
#endif
    ...
    if (is_multi_container::value) {
      ...
      while (s != e) {
        ...
        if (c < 0) {
          ...
          e = mid;
          if (c == 0) {
            // Need to return the first value whose key is not less than
            // k, which requires continuing the binary search if this is a
            // multi-container.
...
template <typename Params> class btree {
  ...
  struct alignas(node_type::Alignment()) EmptyNodeType : node_type {
    ...
    // regarded as a leaf node). max_count() is never called when the tree is
    // empty.
...
private:
  ...
  void deallocate(const size_type sz, node_type *node) {
    phmap::priv::Deallocate<node_type::Alignment()>(mutable_allocator(), node,
                                                    (size_t)sz);
  }
  ...
  // when there is an exact match. The second version is for the key-compare-to
  ...
  if (this != &x) {
    ...
    if (phmap::allocator_traits<
      ...
      if (allocator() == x.allocator()) {
        ...
        // different so we can't take over its memory. We must move each
        // element individually. We need both `x` and `this` to have `x`s
        // key comparator while moving the values so we can't swap the key
        // comparators.
        *mutable_key_comp() = x.key_comp();
        copy_or_move_values_in_order(&x);
      }
  ...
  if (node != root()) {
    ...
    if (node->position() < parent->count()) {
      ...
      if (right->count() < kNodeValues) {
        // We bias rebalancing based on the position being inserted. If
        // we're inserting at the _beginning of the left node then we bias
        // rebalancing to fill up the right node.
        int to_move =
...
public:
  ...
  template <typename K = key_type> iterator lower_bound(const key_arg<K> &key) {
  ...
  template <typename K = key_type> iterator upper_bound(const key_arg<K> &key) {
...
public:
  ...
  std::pair<iterator, bool> try_emplace(key_type &&k, Args &&...args) {
    ...
    // into a forwarding reference and insert_unique guarantees that `key` is
...
class btree_map
    : public priv::btree_map_container<priv::btree<
          priv::map_params<Key, Value, Compare, Alloc, /*TargetNodeSize=*/256,
                           /*Multi=*/false>>> {
...
class btree_multimap
    : public priv::btree_multimap_container<priv::btree<
          priv::map_params<Key, Value, Compare, Alloc, /*TargetNodeSize=*/256,
                           /*Multi=*/true>>> {
