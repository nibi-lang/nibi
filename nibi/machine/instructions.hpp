...
private:
  ...
  inline ins_data_s get_ins_data
    ...
    if (block_idx >= blocks.size()) {
      // TODO: Hook into some system exception/error system 
      //       to gracefully shutdown, or attempt to recover
      std::cerr << "Block index error: " << block_idx << " out of bounds";
      std::exit(1);
    }
    ...
    if (ins_idx >= current_bytes.size()) {
      // TODO: Hook into some system exception/error system 
      //       to gracefully shutdown, or attempt to recover
      std::cerr << "Instruction index error: " << block_idx << ":" << ins_idx << " out of bounds";
      std::exit(1);
    }
